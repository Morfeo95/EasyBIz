...
const BusinessSection = ({ businesses, openModal, onEditBusiness, onDeleteBusiness }) => {
  // Hook de traducciones al inicio para mantener orden de hooks
  const messages = useTranslations();

  // Mostrar loading mientras no haya traducciones
  if (!messages?.perfil?.businessSection) {
    return <div>Loading...</div>;
  }

  // Extraer textos de traducción
  const { title, button: buttonText } = messages.perfil.businessSection;
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6 max-[480px]:flex-col max-[480px]:items-start max-[480px]:gap-4">
        <h2 className="text-2xl font-bold text-gray-800 max-[480px]:text-xl">
          {title}
        </h2>
        <motion.button
