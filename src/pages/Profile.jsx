...
const Profile = () => {
  ...
  // Efecto para cargar datos: negocios, productos y estimados
  useEffect(() => {
    (async () => {
      try {
        // Cargar negocios y productos
        const biz = await fetchBusinesses();
        setBusinesses(biz);
        const productPages = await Promise.all(
          biz.map(b =>
            fetchProductsByBusiness(b.id, { page: 0, size: 100 }).then(r => r.content)
          )
        );
        setProducts(productPages.flat());

        // Cargar estimados (se usa user.id)
        if (user?.id) {
          const estimadosResponse = await fetchEstimados(user.id, 0, 100);
          setEstimados(estimadosResponse.content || estimadosResponse);
        }
      } catch (err) {
  ...
  // Tabs a mostrar en la UI
  const tabs = ['dashboard', 'businesses', 'products', 'estimados'];
  ...
  return (
    <div className="flex flex-col md:flex-row bg-gray-100 min-h-screen">
      ...
      <div className="flex-1 md:ml-64 p-4 md:p-6 space-y-6 mt-16 md:mt-0">
        ...
        <AnimatePresence>
        {modalType === 'business' && (
          <Modal isOpen onClose={() => { setEditingBusiness(null); closeModal(); }} title={editingBusiness ? "Editar negocio" : "Agregar negocio"}>
            <BusinessForm
          ...
          {modalType === 'product' && (
            <Modal isOpen onClose={closeModal} title="Agregar producto">
              <ProductForm
                productId={prodId}
                businesses={businesses}
